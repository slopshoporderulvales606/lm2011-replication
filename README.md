...
Columns (2) and (4) report two weightings of the LM negative-word list. (1) Fin-Neg is the proportional measure: the number of words in a filing that appear in the negative-word list, divided by the filing’s total word count; (2) **Fin-Neg tf-idf** reweights those same words using tf-idf, as defined below (Eq. 1 in the paper):
